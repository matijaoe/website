<script lang="ts" setup>
useSeoMeta({
  title: 'About',
  ogTitle: 'About',
})

const { isDark } = useTheme()
</script>

<template>
  <div>
    <PageHeading>
      About
    </PageHeading>

    <div class="flex flex-col gap-16 max-w-xl">
      <section class="text-default-foreground max-w-xl leading-7 prose text-base md:text-lg">
        <p>
          I'm a software engineer with a sharp eye for UX and design, meticulous attention to detail, and a relentless focus on quality.
        </p>

        <p>
          Skilled in
          <IconWithText text="Vue" icon="logos:vue" />,
          <IconWithText text="Nuxt" icon="logos:nuxt-icon" />,
          and full-stack
          <IconWithText text="TypeScript" icon="logos:typescript-icon" />, I turn ideas into refined, user-friendly experiences. I'm driven by a deep curiosity and a passion for UX/UI design, developer experience, and writing clean, maintainable code.
        </p>

        <p>
          Beyond code, I'm obsessed with
          <IconWithText
            text="Bitcoin"
            icon="logos:bitcoin"
            icon-size="0.9em"
            icon-class="translate-y-0.5"
          /> and a strong advocate for the right to privacy, freedom of speech, self-sovereignty, and open-source software. I'm also deeply interested in economics, cryptography, psychology, and literature.
        </p>
      </section>

      <section class="text-default-foreground max-w-xl leading-7 prose text-base md:text-lg">
        <h2 class="text-default-foreground font-display font-medium mb-4">
          Tech stack
        </h2>
        <p>
          On a regular basis, I work with
          <IconWithText text="Vue" icon="logos:vue" />,
          <IconWithText text="Nuxt" icon="logos:nuxt-icon" />,
          <IconWithText text="TypeScript" icon="logos:typescript-icon" />,
          <IconWithText text="Pinia" icon="logos:pinia" />,
          <IconWithText text="Tailwind" icon="logos:tailwindcss-icon" />,
          <IconWithText text="VueUse" icon="logos:vueuse" />,
          <IconWithText text="Vue Router" icon="logos:vue" />,
          <IconWithText text="Vite" icon="logos:vitejs" />,
          <IconWithText text="Vitest" icon="logos:vitest" />,
          <IconWithText text="GraphQL" icon="logos:graphql" />,
          <IconWithText text="Apollo" icon="simple-icons:apollographql" /> and
          <IconWithText text="pnpm" icon="logos:pnpm" />. 
        </p>

        <p>
          I also semi-regularly use <IconWithText text="Tanstack Query" icon="logos:react-query-icon" />, <IconWithText text="Storybook" icon="logos:storybook-icon" />, <IconWithText text="Radix UI" icon="simple-icons:radixui" /> and <IconWithText text="Playwright" icon="logos:playwright" />.
        </p>

        <p>
          On my own projects, I usually stick to the Vue ecosystem, with <IconWithText text="Tailwind" icon="logos:tailwindcss-icon" /> or <IconWithText text="UnoCSS" icon="logos:unocss" /> for styling, often pairing them with <IconWithText text="shadcn" icon="simple-icons:shadcnui" /> or <IconWithText text="Nuxt UI" icon="logos:nuxt-icon" /> for UI components.
        </p>

        <p>
          On the backend, I primarily use <IconWithText text="Node" icon="logos:nodejs-icon" /> or <IconWithText text="Bun" icon="logos:bun" />, with <IconWithText text="Nitro" icon="unjs:nitro" /> or <IconWithText text="Hono" icon="logos:hono" /> as web frameworks, and an SQL database with <IconWithText text="Drizzle" icon="simple-icons:drizzle" /> or <IconWithText text="Prisma" icon="simple-icons:prisma" />. For scripting and CLI tools, it's usually just <IconWithText text="Bun" icon="logos:bun" />, less often <IconWithText text="Deno" icon="logos:deno" />.
        </p>

        <p>
          I also have some experience building projects with <IconWithText text="React" icon="logos:react" />, <IconWithText text="Quasar" icon="devicon:quasar" />, or vanilla <IconWithText text="JavaScript" icon="logos:javascript" />, along with some smaller projects using <IconWithText text="Svelte" icon="logos:svelte-icon" />. I’ve also built backends and APIs using <IconWithText text="Nest.js" icon="logos:nestjs" /> and <IconWithText text="Express" icon="simple-icons:express" />. 
        </p>

        <p>
          Some notable libraries I enjoy working with are: <IconWithText text="Zod" icon="simple-icons:zod" />, <IconWithText text="VeeValidate" icon="devicon:veevalidate" />, date-fns, <IconWithText text="ESLint" icon="logos:eslint" />, <IconWithText text="ofetch" icon="unjs:ofetch" />, <IconWithText text="h3" icon="unjs:h3" />, <IconWithText text="bitcoinjs-lib" icon="simple-icons:bitcoin" />, <IconWithText text="Headless UI" icon="logos:headlessui-icon" />, <IconWithText text="Mantine" icon="logos:mantine-icon" />.
        </p>

        <h3 class="text-default-foreground font-display font-medium text-2xl">
          On my radar
        </h3>

        <p>
          I haven't explored <IconWithText text="Astro" icon="simple-icons:astro" /> and <IconWithText text="SolidJS" icon="logos:solidjs-icon" /> as much as I probably should, and I really want to improve my animation skills—I've been putting off learning <IconWithText text="GSAP" icon="logos:greensock-icon" /> and <IconWithText text="Motion" icon="simple-icons:framer" /> for far too long.
        </p>

        <h3 class="text-default-foreground font-display font-medium text-2xl">
          Beyond web
        </h3>

        <p>
          Outside of web dev, <IconWithText text="Python" icon="logos:python" /> is my go-to language. I'm definitely a bit rusty, but I'm starting to pick it up again. Occasionally I use <IconWithText text="Bash" icon="logos:bash-icon" /> for scripting, though it often ends up with ChatGPT doing it for me.
        </p>

        <p>
          I worked extensively with <IconWithText text="Java" icon="logos:java" /> during my college years but haven't touched it in a while. a few years ago, I experimented with <IconWithText text="Solidity" icon="simple-icons:solidity" /> , but later realized there was no future in <Icon name="simple-icons:ethereum" aria-label="Ethereum" title="Ethereum" />. More recently, I've been exploring <IconWithText text="ReScript" icon="logos:rescript-icon" />, a statically-typed language that compiles to efficient and human-readable JavaScript.
        </p>

        <p>
          In the future, I'd love to explore functional programming, particularly <IconWithText text="Elixir" icon="devicon:elixir" /> or <IconWithText text="Ocaml" icon="devicon:ocaml" />.
        </p>
      </section>
    </div>
  </div>
</template>
