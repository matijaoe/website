/* Vendored WebGL fluid simulation — kept close to its original form, so the
   house style rules are relaxed for this file only. */
...
/**
 * Vendored WebGL fluid simulation.
 * Returns a cleanup function that cancels the rAF loop, removes listeners
 * and drops the GL context.
 *
 * @param {HTMLCanvasElement} canvas
 * @param {{ isDark: () => boolean }} options - getter, so the sim picks up
 *   theme changes without importing anything from Vue.
 * @returns {() => void} Cleanup function.
 */
export function createFluidCanvas(canvas, { isDark }) {
  const cleanupFns = []
...
`)
  ...
  function generateColor() {
    const configHue = isDark() ? config.HUE_DARK : config.HUE_LIGHT
